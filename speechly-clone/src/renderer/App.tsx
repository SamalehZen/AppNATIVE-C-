...
export const App: React.FC = () => {
  ...
  }, [isListening, stopListening, startListening, transcript, cleanup, language, settings, resetTranscript, resetCleanup]);
  ...
  useEffect(() => {
    ...
    if (window.electronAPI?.onToggleRecording) {
      (window.electronAPI as any).onToggleRecording(handleHotkey);
    }
    
    return () => {
      if (window.electronAPI?.removeToggleRecordingListener) {
        (window.electronAPI as any).removeToggleRecordingListener();
      }
    };
  }, [handleToggleRecording]);
  ...
  const wordCount = (cleanedText || transcript).split(/\s+/).filter(w => w.length > 0).length;
    ...
    <div className="h-screen flex flex-col bg-bg-primary">
      <header className="draggable flex items-center justify-between px-6 py-4 border-b border-bg-tertiary">
        <div className="flex items-center gap-4 pl-16">
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg font-semibold text-text-primary">Speechly</span>
          </div>
          <StatusIndicator
        ...
        <button
          ...
          className="non-draggable p-2 rounded-lg hover:bg-bg-tertiary transition-colors"
        >
      ...
      <main className="flex-1 flex flex-col items-center p-6 overflow-hidden">
        <div className="flex flex-col items-center gap-4 mb-6">
          ...
          />
          {!isSupported && (
            <p className="text-red-400 text-sm">Speech recognition is not supported in this browser.</p>
          )}
          {speechError && (
